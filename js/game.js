...
function carregarJogo(game) {
  ...
  document.getElementById("game-desenvolvedora").textContent =
    game.desenvolvedora;
  document.getElementById("game-distribuidora").textContent =
    game.distribuidora;
  document.getElementById("game-idade-recomendada").textContent =
    game.idade_recomendada;
  ...
  document.getElementById("game-campeonatos").textContent =
    game.campeonatos.join(", ");
    ...
    if (!mesmoDia) {
      if (diffMin > 0) {
        ...
        btns[index].innerHTML =
          'Transmissão não disponível <i class="fa-solid fa-lock ms-1"></i>';
        btns[index].disabled = true;
      } else {
        btns[index].innerHTML =
          'Assistir Gravação <i class="fa-solid fa-play"></i>';
      }
    } else {
      btns[index].innerHTML =
        'Assistir Transmissão <i class="fa-solid fa-play"></i>';
    }
  });
}
...
function carregarFiltros(game) {
  ...
  addTimesFiltro(
    game.partidas.map((partida) => partida.time1),
    game.partidas.map((partida) => partida.time2)
  );
}
...
function aplicarFiltros(game) {
  const campeonatoSelecionado =
    document.getElementById("campeonatos-game").value;
  ...
  const partidasFiltradas = game.partidas.filter((partida) => {
    const campeonatoOk =
      campeonatoSelecionado === "Todos" ||
      campeonatoSelecionado === "Campeonatos" ||
      partida.campeonato === campeonatoSelecionado;
    const timeOk =
      timeSelecionado === "Todos" ||
      timeSelecionado === "Times" ||
      partida.time1 === timeSelecionado ||
      partida.time2 === timeSelecionado;
...
function carregarPagina() {
  ...
  document
    .getElementById("campeonatos-game")
    .addEventListener("change", () => aplicarFiltros(game));
  document
    .getElementById("campeonatos-time")
    .addEventListener("change", () => aplicarFiltros(game));
