...
export default function GamePage() {
  ...
  const { isFavorite, loadingFavorite, checkFavorite, toggleFavorite } =
    useFavoriteToggle({
      user,
      token,
      entityId: gameData?.id,
      entityData: gameData,
      notificationType: NotificationType.GAME,
    });

  const championshipsLoaded = useRef(false);

  const handleGetInfoGame = useCallback(async (id) => {
    try {
      setLoading(true);
  ...
  const handleGetInfoChampionships = useCallback(async (gameId) => {
    try {
  ...
  }, [router, handleGetInfoGame]);

  useEffect(() => {
    if (gameData?.id && !championshipsLoaded.current) {
      handleGetInfoChampionships(gameData.id);
    }
  }, [gameData?.id, handleGetInfoChampionships]);

  useEffect(() => {
    if (gameData?.id) {
      checkFavorite();
    }
  }, [gameData?.id, checkFavorite]);
